...
static bool chunk_can_be_aligned(const struct memhdr *chunk, size_t allocsize, size_t align)
{
	void *p = chunk2mem(chunk);
	void *alignedp = ROUND_UP(p, align);
	...
	if (diff >= chunk->size || diff < MIN_CHUNK_SIZE)
		return false;
	if (chunk->size - diff < n)
		return false;

	return true;
}

static struct memhdr *find_bestfit_aligned(struct memhdr *list, size_t n, size_t align)
{
	...
	do {
		ft_assert(!cur->cinuse);
		void *p = chunk2mem(cur);

		if (cur->size == n && IS_ALGINED_TO(p, align)) {
			return cur;
		} else if (cur->size > n && chunk_can_be_aligned(cur, n, align)) {
